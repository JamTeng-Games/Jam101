...
// Unity-based, but could be adapted to work in pure C#
    public class PointOctree<T>
    {
        ...
        PointOctreeNode<T> _rootNode;

        public Dictionary<T, Vector3> NodePrevPosDict { get; private set; }
        // Size that the octree was on creation
        readonly float _initialSize;
        ...
        public PointOctree(float initialWorldSize, Vector3 initialWorldPos, float minNodeSize)
        {
            ...
            _rootNode = new PointOctreeNode<T>(_initialSize, _minSize, initialWorldPos);
        }
        ...
        public void Add(T obj, Vector3 objPos)
        {
            ...
            NodePrevPosDict.Add(obj, objPos);
            Count++;
        }
        ...
        public void GetNearby(Vector3 position, float maxDistance, List<T> ret)
        {
            _rootNode.GetNearby(ref position, maxDistance, ret);
        }
        ...
        /// <summary>
        /// Returns objects that are within <paramref name="testBounds"/>.
        /// If none, returns false. Uses supplied list for results.
        /// </summary>
        /// <param name="maxDistance">Maximum distance from the position to consider</param>
        /// <param name="res">Pre-initialized list to populate</param>
        /// <returns>True if items are found, false if not</returns>
        public bool GetByBoundsNonAlloc(in Bounds testBounds, List<T> res)
        {
            res.Clear();
            _rootNode.GetByBoundsNonAlloc(in testBounds, res);
            if (res.Count > 0)
                return true;
            return false;
        }
        ...
        {
            Profiler.BeginSample("octree grow");
            int xDirection = direction.x >= 0 ? 1 : -1;
            int yDirection = direction.y >= 0 ? 1 : -1;
            int zDirection = direction.z >= 0 ? 1 : -1;
            PointOctreeNode<T> oldRoot = _rootNode;
            ...
            _rootNode = new PointOctreeNode<T>(newLength, _minSize, newCenter);
            ...
            if (oldRoot.HasAnyObjects())
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    else
                    {
                        ...
                        children[i] = new PointOctreeNode<T>(oldRoot.SideLength, _minSize,
