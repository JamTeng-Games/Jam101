...
namespace J.Core
{
    ...
    // Event type
    public class JEvent<TEventType> where TEventType : Enum
    {
        ...
        private JEvent<TEventType> _parent;
        private bool IsAttached => _parent != null;

        public static JEvent<T> Create<T>() where T : Enum
        {
            JEvent<T> t = new JEvent<T>();
            return t;
        }

        private JEvent()
        {
            _transDic0 = new Dictionary<TEventType, List<IEventCallback>>();
            _transDic1 = new Dictionary<TEventType, List<IEventCallback>>();
            _transDic2 = new Dictionary<TEventType, List<IEventCallback>>();
            _transDic3 = new Dictionary<TEventType, List<IEventCallback>>();
            _commands = new List<EventCommand>();
        }
        ...
        // 这样只要在根Signal上注册事件
        public void Attach(JEvent<TEventType> parent)
        {
            _parent = parent;
        }
