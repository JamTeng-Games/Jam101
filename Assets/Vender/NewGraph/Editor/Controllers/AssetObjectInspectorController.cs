...
namespace NewGraph
{
    ...
    public class AssetObjectInspectorController : InspectorControllerBase
    {
        public AssetObjectInspectorController(VisualElement parent) : base(parent)
        {
        }
        ...
        private void CreateButtonClicked()
        {
            ...
            string lastFolder = Application.dataPath;
            if (lastOpenedDirectoryGUID != null)
            {
                lastFolder = AssetDatabase.GUIDToAssetPath(lastOpenedDirectoryGUID);
                if (lastFolder == null)
                {
                    lastFolder = Application.dataPath;
                }
            }

            // generate a unique filename that avoids conflicts
            string uniqueFilename = Path.Combine(lastFolder, typeof(AssetObjectGraphModel).Name + "." + fileEnding);
            ...
            if (path.Length != 0)
            {
                UnityEngine.Object graphData = ScriptableObject.CreateInstance<AssetObjectGraphModel>();
        ...
        private void LoadButtonClicked()
        {
            ...
            EditorGUIUtility.ShowObjectPicker<AssetObjectGraphModel>(null, false, "", currentPickerWindow);
            pickerActive = true;
        }
        ...
        {
            AssetObjectGraphModel graphModel = graph as AssetObjectGraphModel;
